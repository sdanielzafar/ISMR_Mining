...
def pickle_data(data,fileName):
	with open(fileName[:-4]+'2.pkl', 'wb') as f:
		pickle.dump(data,f)
...
def main():
	...
	sstData = parse_sst(sstData)[:,564:-74]
	imp = Imputer(missing_values='NaN', strategy='mean', axis=0)
	imp.fit(sstData)
	sstData = imp.transform(sstData)
	sstData = preprocessing.scale(sstData)
	pickle_data(sstData,sstFile)
